...
Are you looking for a way to test the latest WebAssembly proposals? Check out `wasmtime`. Ditto if you need to [run WebAssembly modules from Zig](https://github.com/kubkon/wasmtime-zig). `wasmtime` is fast, even on ARM CPUs, although I wish I could test it on one of these fancy new Apple M1 machines.
